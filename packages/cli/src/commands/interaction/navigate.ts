...
export default defineCommand({
  meta: { description: 'Add a click-to-navigate interaction' },
  args: {
    id: { type: 'positional', description: 'Source node ID', required: true },
    destination: { type: 'positional', description: 'Destination node ID', required: true },
    trigger: { type: 'string', description: 'Trigger type', default: 'ON_CLICK' },
  ...
  async run({ args }) {
    try {
      ...
      console.log(`Added interaction #${index}: ${args.trigger.toUpperCase()} → NAVIGATE → ${args.destination}`)
    } catch (e) {
      handleError(e)
    }
  }
