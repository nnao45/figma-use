...
export default defineCommand({
  meta: { description: 'Create a scatter chart' },
  args: {
    data: { type: 'string', description: 'Data (x:y,...)', required: true },
    ...
    color: { type: 'string', description: 'Point color (hex)', default: '#3B82F6' },
    'point-size': { type: 'string', description: 'Point size', default: '6' },
  ...
  async run({ args }) {
    try {
      ...
      g.selectAll('circle')
        ...
        .attr('r', pointSize)
        .attr('fill', (_: ScatterPoint, i: number) => colors[i % colors.length]!)
