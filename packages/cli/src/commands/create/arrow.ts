...
export default defineCommand({
  meta: { description: 'Create an arrow (line with customizable end caps)' },
  args: {
  ...
  async run({ args }) {
    try {
      ...
      const endCap = args['end-cap'] ? String(args['end-cap']).toLowerCase() : 'arrow'
      ...
      const result = await sendCommand('create-arrow', {
        x: Number(args.x),
        y: Number(args.y),
