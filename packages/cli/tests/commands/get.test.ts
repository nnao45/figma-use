...
describe('get', () => {
  ...
  beforeAll(async () => {
    ...
    const comp = await run('create component --x 0 --y 0 --width 100 --height 50 --name "TestGetComp" --json') as any
    componentId = comp.id
  ...
  test('components returns array', async () => {
    const components = await run(`get components --page "${testPageId}" --limit 10 --json`) as any[]
    expect(Array.isArray(components)).toBe(true)
  })

  test('components filters by name', async () => {
    const components = await run(`get components --page "${testPageId}" --name "TestGetComp" --limit 10 --json`) as any[]
    expect(components.length).toBeGreaterThanOrEqual(1)
    expect(components.some(c => c.name === 'TestGetComp')).toBe(true)
  })

  test('components respects limit', async () => {
    const components = await run(`get components --page "${testPageId}" --limit 5 --json`) as any[]
